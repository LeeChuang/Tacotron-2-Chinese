...
&ensp; &ensp; 用根目录的 `sentences.txt` 中的文本合成语音。

* 使用 WaveNet 作为声码器（极慢，高音质）：

   ```Shell
   python synthesize.py --model='Tacotron-2' --text_list='sentences.txt'
   ```

   生成的语音位于 `/wavenet_output/wavs/` 中。

* 使用 Griffin-Lim 作为声码器（快，低音质）：
  
   ```Shell
   python synthesize.py --model='Tacotron-2' --text_list='sentences.txt' --mode='live'
   ```

   生成的语音位于 `/tacotron_output/wavs/` 中。
